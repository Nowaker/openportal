...
export default defineHandler(async (event) => {
  ...
  const directory =
    typeof body?.directory === "string" && body.directory.length > 0
      ? body.directory
      : undefined;
  const cleanBeforeCompaction = body?.cleanBeforeCompaction === true;
  if (!sessionId) {
  ...
  try {
    ...
    return {
      ok: false,
      error: err instanceof Error ? err.message : "failed to start stuck-compaction-fixer",
    };
  }
});
